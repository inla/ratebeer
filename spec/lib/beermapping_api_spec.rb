...
describe "BeermappingApi" do
  describe "in case of cache miss" do
    ...
    it "When HTTP GET returns no entries, an empty array is returned" do
      ...
      stub_request(:get, /.*lohja/).to_return(body: canned_answer, headers: { 'Content-Type' => "text/xml" })

      places = BeermappingApi.places_in("lohja")

      expect(places.size).to eq(0)
    end

    it "When HTTP GET returns many entries, they are all parsed and returned" do
      canned_answer = <<-END_OF_STRING 
      ...
      place = places.first
      expect(place.name).to eq("Saimaan Juomatehdas")
      expect(place.street).to eq("Meijerinkatu 4")
      place = places.last
      expect(place.name).to eq("Tuju")
      expect(place.street).to eq("Meijerinkatu 4")
    end
